...
export function useDashboard() {
  const { data: overview, status: overviewStatus } = useAsyncData(
    'dashboard-overview',
    fetchOverview,
    { server: false }
  )
  const { data: portfolio, status: portfolioStatus } = useAsyncData(
    'dashboard-portfolio',
    fetchPortfolio,
    { server: false }
  )
  const { data: spending, status: spendingStatus } = useAsyncData(
    'dashboard-spending',
    fetchSpending,
    { server: false }
  )
  const { data: income, status: incomeStatus } = useAsyncData(
    'dashboard-income',
    fetchIncome,
    { server: false }
  )
  const { data: byAccount, status: byAccountStatus } = useAsyncData(
  ...
  const activeIncome = computed(
    () => activeEntry.value?.income ?? income.value
  )
